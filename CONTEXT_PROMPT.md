...
- [CP-2.27] **Telegram Mini Apps (TMA) Integration**: Secure, personalized Web UI serving as a full-featured **«Personal Cabinet»**. It mirrors the Telegram Start Menu with 100% functional parity, providing dedicated views for "My Topics", "Events", and "Profile", along with **Administrative Tools** (Global Topics, Access Templates, Roles FAQ) for authorized users. Includes HMAC cryptographic validation, mobile-native haptic feedback, and a robust multi-view navigation system using the Telegram BackButton. [CC-5]
