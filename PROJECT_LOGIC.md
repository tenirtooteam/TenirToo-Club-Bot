...
- **services/ui_service.py** — Centralized UI lifecycle via `UIService` class (all methods `@staticmethod`): `clear_last_menu`, `delete_msg`, `finish_input`, `send_redirected_menu`, `show_menu`, `generic_navigator` (central UI router), `show_admin_dashboard`, `show_moderator_dashboard`, `ask_input`, `show_temp_message`, `show_user_detail`, `show_group_detail`, `show_topic_detail`, `show_moderator_groups`, `show_moderator_moderators`, `sterile_command`, `get_confirmation_ui`, `format_user_card`.
- **services/management_service.py** — Centralized Domain Service for entity management. **Contract**: All methods return a `(bool, str)` tuple. **Search-Or-Action Protocol**: Methods like `assign_moderator_role` and `grant_direct_access` return `False, "SEARCH_REQUIRED"` if an unambiguous ID is not found, signaling the handler to trigger search disambiguation. **Functions**: `ensure_user_registered` (shadow auto-reg), `add_user`, `create_group`, `assign_moderator_role`, `grant_direct_access`.
...
- **handlers/admin.py** — Admin flows protected by `IsGlobalAdmin`. FSM states: `waiting_for_group_name`, `waiting_for_topic_name`, `waiting_for_user_data`, `waiting_for_new_name`, `waiting_for_new_role_user`. Functions: `admin_dashboard`, `back_to_main`, `show_groups`, `role_assign_choose_topic`, `group_detail`, `add_group_init`, `process_group_add`, `delete_group_init`, `show_all_topics`, `topic_detail`, `show_group_topics`, `remove_topic_from_group_init`, `add_topic_to_group_init`, `confirm_add_topic`, `topic_rename_init`, `process_topic_name_save`, `global_topic_delete_init`, `show_users`, `add_user_init`, `process_user_add`, `user_detail`, `user_rename_init`, `process_user_rename`, `user_groups_ui`, `toggle_group`, `user_delete_init`, `role_assign_choose_user`, `user_roles_manage_handler`, `role_pick_handler`, `role_assign_topic_confirm`, `role_revoke_init`. Delegated UI transitions to `UIService.generic_navigator`.
...
- **UIService.generic_navigator**: Unified entry point for all UI transitions. Maps callback data strings to specific `UIService` show methods or keyboard builders. Ensures consistent parameter parsing and navigation state management.
