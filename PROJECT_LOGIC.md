...
- **database/members.py** — Transactional CRUD for users: `add_user`, `delete_user`, `update_user_name` (allows optional empty `last_name`), `get_user_name`, `get_all_users`, `user_exists`, `find_users_by_query` (case-insensitive substring search on `first_name`/`last_name` fields; filtering is performed in Python rather than SQL because SQLite's `LOWER()` does not handle Cyrillic characters correctly; accepts a 1- or 2-word query string; returns a list of `(user_id, first_name, last_name)` tuples). Note: `delete_user` issues a single `DELETE FROM users` — cascade to `user_roles`, `group_members`, and `direct_topic_access` is handled at the DB level via `ON DELETE CASCADE`.
- **database/topics.py** — Transactional CRUD for topics (registration, renaming, deletion): `get_all_unique_topics`, `update_topic_name`, `get_topic_name`, `delete_topic`, `register_topic_if_not_exists`, `find_topics_by_query`, `get_topic_names_by_ids`.
- **database/groups.py** — Group template operations. Functions: `create_group`, `get_all_groups`, `get_group_name`, `delete_group`, `get_topics_of_group`, `add_topic_to_group`, `remove_topic_from_group`, `get_groups_by_topic`, `find_groups_by_query`, `add_to_group_template`, `remove_from_group_template`, `get_group_template_members`, `get_user_group_templates`, `get_user_group_membership_ids`, `get_group_ids_by_topic`.
...
- **database/permissions.py** — Direct per-topic access management. Functions: `grant_direct_access`, `grant_direct_access_bulk`, `revoke_direct_access`, `revoke_all_direct_access`, `get_direct_access_users`, `has_direct_access`, `can_write`, `is_topic_restricted`, `get_topic_authorized_users`, `get_user_available_topics`, `get_direct_access_user_ids`, `get_topic_authorized_user_ids`.
...
- **services/ui_service.py** — Centralized UI lifecycle via `UIService` class (all methods `@staticmethod`): `clear_last_menu` (Multi-Message Stack cleanup via `last_menu_ids`), `delete_msg`, `finish_input`, `send_redirected_menu`, `show_menu`, `generic_navigator` (central UI router using `PAGINATED_CMDS` constant for explicit call signatures), `show_admin_dashboard`, `show_moderator_dashboard` (fixed signature without `text` param), `ask_input`, `show_temp_message` (adds message to cleanup stack), `show_user_detail`, `show_group_detail`, `show_topic_detail`, `show_moderator_groups`, `show_moderator_moderators`, `sterile_command`, `get_confirmation_ui`, `format_user_card`.
- **services/management_service.py** — Centralized Domain Service for entity management. **Contract**: All methods MUST return a `(bool, str)` tuple. **Search-Or-Action Protocol**: Methods like `assign_moderator_role` and `grant_direct_access` return `False, "SEARCH_REQUIRED"` if an unambiguous ID is not found, signaling the handler to trigger search disambiguation. **Functions**: `ensure_user_registered` (shadow auto-reg), `add_user` (flexible space-aware parsing for multi-word names), `create_group`, `assign_moderator_role`, `assign_moderator_role_by_id` (optimized by-id entry), `grant_direct_access`, `grant_direct_access_by_id` (optimized by-id entry), `toggle_user_group_template` (manages non-runtime membership/template), `apply_group_to_topic` (bulk copy from template to direct access), `sync_group_to_topic` (full sync: wipe topic direct access + copy from template), `copy_topic_to_topic` (access transfer), `grant_role`, `execute_deletion`, `update_user_name`.
...
- **handlers/common.py** — Unfiltered router (no role filter). Private helper `_fetch_search_results(s_type, query)` — single centralized point for dispatching search queries across user/group/topic entity types, eliminating triple duplication. Global `close_menu` callback handler. Global role-aware `/help` command (`cmd_help`). **Roles Dashboard** handlers: `roles_dashboard_menu` (informational center for admins and moderators), `roles_faq_view` (role description guide), `list_users_with_roles` (overview of all active assignments). **Unified Search** handlers: `search_start_handler` (initiates type-aware search prompt), `search_query_handler` (processes input, delegates to `_fetch_search_results`, auto-selects single results), `search_results_pagination` (paginates search results), `search_pick_handler` (delegates to `perform_search_pick`), `perform_search_pick` (universal post-selection router), `confirm_execution` (systemic deletion point).
...
- **handlers/moderator.py** — Moderator flows protected by `IsTopicManager`. FSM states: `waiting_for_topic_name`, `waiting_for_user_data`, `waiting_for_direct_access_user`. Functions: `moderator_dashboard`, `moderator_topic_select`, `moderator_back_to_topic`, `moderator_rename_topic_init`, `moderator_rename_topic_finish`, `moderator_show_groups`, `moderator_link_group`, `moderator_unlink_group_init`, `moderator_unlink_group_confirm`, `moderator_manage_users`, `moderator_add_user_list`, `moderator_toggle_direct_access`, `process_direct_access_user_search` (delegates SEARCH_REQUIRED to `common.py`), `moderator_show_moderators`, `moderator_add_moderator_start`, `moderator_remove_moderator_init`. Delegated UI transitions to `UIService.generic_navigator`.
- **handlers/user.py** — User-facing flows: `/start`, `back_to_user_main`, `user_profile_callback`, `show_user_topics`, `user_topic_detail`, `handle_all_mention`. All handlers declare `state: FSMContext` in signature where `UIService` navigation methods are called. Delegated UI transitions to `UIService.generic_navigator`.
...
- **UIService.generic_navigator**: Unified entry point for all UI transitions. Maps callback data strings to specific `UIService` show methods or keyboard builders. Supports global panels (Admin, Moderator, User), profile views, and topic details. Uses the `PAGINATED_CMDS` class constant to explicitly determine if a keyboard requires the `page` argument, eliminating unsafe `try/except` signature guessing. Includes fallback logging for unknown commands. `[AI-1]` Standard: All standard UI returns and transitions MUST traverse this router.
