...
The complete file list with individual responsibilities and full function inventory is reference data.
> Moved to docs/knowledge/module-registry.md — read on demand.
...
The full SQLite DDL (tables, foreign keys, indexes) is reference data.
> Moved to docs/knowledge/db-schema.md — read on demand.
...
- [PL-5.1.18] **Quick Announcement Protocol**: Support for `/an` command in forum topics. Automatically creates a "Rapid Event" (date set to 'Оперативно') and posts a rich announcement. **Shortened Layout [CC-2]**: If the date is 'Оперативно', the date line is hidden in the announcement for maximum clarity. Uses "📍 Топик" terminology. **Dynamic Reactivity**: Any interaction with the announcement (joining/leaving) triggers a real-time text update with the current participant list. Original command message is deleted to maintain thread sterility.
