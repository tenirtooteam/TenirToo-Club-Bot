...
- [PL-2.2.21] **services/notification_service.py** — Notification logic (feature 013 №18/№5): `send_native_all` (moderator/superadmin-gated @all, per-sender cooldown, mention batching by `MENTION_BATCH_SIZE`), `send_to_users` (Targeted Broadcast with flood-wait retry + pacing via `_send_message_resilient`), rate-limited deny alerts with bounded `_alert_cache` (`_prune_alert_cache`). Test hook: `reset_notification_state`.
